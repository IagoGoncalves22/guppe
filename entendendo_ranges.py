"""
Ranges

- Precisamos conhecer o loop for para usar os ranges.
- Precisamos conhecer melhor o range para trabalhar melhor com loop for.

Ranges são utilizados para gerar sequências numéricas, não de forma
 aleatória,mas sim de maneira especificada

Formas gerais:

# Forma 1
range(valor_de_parada)

OBS: valor_de_parada não inclusive (início padrão 0, e passo de 1 em 1)

# Exemplo forma 1
for num in range(11):
    print(num)

#Froma2
range(valor_de_inicio, valor_de_parada)

OBS: valor_de_parada não inclusive (inicio  especificado pelo usuário,
e passado de 1 em 1).

# Exemplo forma 2
for num in range(1, 11):
    print(num)

# Forma 3

range(valor_de_inicio, valor_de_parada, passo)

OBS: valor_de_parada não inclusive (inicio  especificado pelo usuário,
e passado especificado pelo usuário).

# Exemplo forma 3
for num in range (5,100, 5):
    print(num)


# Forma 4 (Inversa)

range(valor_de_inicio, valor_de_parada, passo)

OBS: valor_de_parada não inclusive (início  especificado pelo usuário,
e passado especificado pelo usuário).

# Exemplo forma 4
for num in range(10, -1, -1):
    print(num)qq

"""

