...
# Formatandop datas/horas com strftime() (String Format Time)
...
# Vale a pena consultar a documentação do Python para formatação de date
hoje_formatado = hoje.strftime('%d/%d/%Y')
...
def formata_data(data):
    if data.month == 1:
        ...
        return f'{data.day} de Desembro de {data.year}'
...
def formata_data(data):
    return f"{data.day} de {TextBlob(data.strftime('%B')).translate(to='pt-br')} de {data.year}" # Não funcionou, translate do TextBlob descontinuando
...
almoco = datetime.time(12, 30, 0) # três parâmetros, hora, minuto, segundo
